      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div>
            ...
            >
              Edit Relasi Gejala - Penyakit
            </h1>
            <p class="text-gray-600">
              Perbarui hubungan antara gejala dan penyakit
            </p>
          </div>
        </div>
      </div>
          ...
          >
            <div class="flex items-center space-x-3">
              ...
              <h3 class="text-lg font-semibold text-gray-800">
                Edit Form Relasi Gejala-Penyakit
              </h3>
            </div>
          </div>
          ...
          <form @submit.prevent="submitForm" class="p-6 space-y-6">
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
              <div class="text-sm text-gray-500">
                Memperbarui relasi gejala-penyakit
              </div>
              <div class="flex space-x-3">
                ...
                <button
                  ...
                  class="group/btn relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-lg font-medium shadow-lg hover:shadow-xl hover:shadow-amber-500/25 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                >
                  <div
                    class="absolute inset-0 bg-gradient-to-r from-amber-600 to-orange-700 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300 rounded-lg"
                  ></div>
                  <svg
                    v-if="submitting"
                  ...
                  <span class="relative">
                    {{ submitting ? 'Memperbarui...' : 'Update Relasi' }}
                  </span>
                </button>
              </div>
            </div>
...
export default {
  name: 'EditRelasiGejalaPenyakit',
  data() {
    return {
      form: {
        penyakit: '',
        gejala: [] // array untuk multiple checkbox
      },
      penyakitList: [],
      gejalaList: [],
      loading: true,
  ...
  methods: {
    ...
    async fetchRelasi() {
      ...
      try {
        const res = await gejalaPenyakitAPI.getById(id);
        this.form.penyakit = res.data.penyakit._id;
        // Ambil array gejala dari populate dan map ke array id
    ...
    async submitForm() {
      ...
      try {
        const id = this.$route.params.id;
        await gejalaPenyakitAPI.update(id, this.form);
        this.showSuccessMessage('Relasi berhasil diperbarui');
        this.$router.push('/admin/gejala-penyakit');
      } catch (err) {
        console.error('Gagal memperbarui relasi:', err);
        this.showErrorMessage('Gagal memperbarui relasi');
      } finally {
