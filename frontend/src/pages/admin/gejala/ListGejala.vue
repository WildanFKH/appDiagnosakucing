  ...
  <div class="relative">
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div>
          <h1
            class="text-3xl font-bold bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 bg-clip-text text-transparent mb-2"
          >
            Daftar Gejala
          </h1>
          <p class="text-gray-600">
            Kelola semua gejala dalam sistem pakar kucing
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-20">
      <div class="relative">
        <div
          class="w-12 h-12 rounded-full border-4 border-green-200 border-t-green-600 animate-spin"
        ></div>
        <div
          class="absolute inset-0 w-12 h-12 rounded-full border-4 border-transparent border-t-emerald-400 animate-spin animation-delay-150"
        ></div>
      </div>
    ...
    <!-- Empty State -->
    <div v-else-if="gejalas.length === 0" class="group relative">
      <div
      ...
      >
        ...
        <h3 class="text-2xl font-bold text-gray-800 mb-3">Belum Ada Gejala</h3>
        <p class="text-gray-600 mb-6">
          Tambahkan gejala pertama untuk memulai sistem diagnosa
        </p>
        <router-link
          to="/admin/gejala/tambah"
          class="inline-flex items-center space-x-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl hover:shadow-green-500/25 transition-all duration-300 transform hover:scale-105"
        >
          ...
          <span>Tambah Gejala Pertama</span>
        </router-link>
      </div>
    </div>
    ...
    <div v-else class="group relative">
      <div
        class="absolute inset-0 bg-gradient-to-br from-green-400/10 via-emerald-400/10 to-teal-400/10 rounded-2xl blur-xl group-hover:blur-2xl transition-all duration-500"
      ></div>
      <div
      ...
      >
        <!-- Table Header -->
        <div
          class="bg-gradient-to-r from-green-50/80 to-emerald-50/80 backdrop-blur-sm border-b border-green-100/50 px-6 py-4"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <h3 class="text-lg font-semibold text-gray-800">Data Gejala</h3>
            </div>
            <div class="flex items-center space-x-4">
              <div
                class="text-sm text-gray-600 bg-white/50 px-3 py-1 rounded-full"
              >
                {{ gejalas.length }} gejala terdaftar
              </div>
              <div
                class="text-sm text-gray-600 bg-white/50 px-3 py-1 rounded-full"
              >
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody>
              <tr
                v-for="(gejala, index) in paginatedGejalas"
                :key="gejala._id"
                class="hover:bg-gradient-to-r hover:from-green-50/30 hover:to-emerald-50/30 transition-all duration-300"
                :class="{ 'bg-white/20': index % 2 === 0 }"
              >
                <td class="px-6 py-6 border-b border-gray-100/30">
                  <div class="flex items-center">
                    <div
                      class="w-8 h-8 bg-gradient-to-r from-green-400/20 to-emerald-500/20 rounded-lg flex items-center justify-center"
                    >
                ...
                <td class="px-6 py-6 border-b border-gray-100/30">
                  <div class="font-medium text-gray-900">{{ gejala.nama }}</div>
                </td>
                <td class="px-6 py-6 border-b border-gray-100/30">
                  <div class="text-gray-600 text-sm">
                    <div
                      v-if="!expandedRows.includes(gejala._id) && gejala.deskripsi.length > 100"
                      class="space-y-2"
                    >
                      <p>{{ gejala.deskripsi.substring(0, 100) }}...</p>
                      <button
                        @click="toggleExpand(gejala._id)"
                    ...
                    <div v-else class="space-y-2">
                      <p class="leading-relaxed">{{ gejala.deskripsi }}</p>
                      <button
                        v-if="gejala.deskripsi.length > 100"
                        @click="toggleExpand(gejala._id)"
                        class="text-green-600 hover:text-green-800 text-xs font-medium underline flex items-center space-x-1"
                      >
                ...
                <td class="px-6 py-6 border-b border-gray-100/30">
                  <div class="flex items-center justify-center space-x-2">
                    <!-- Edit Button -->
                    <router-link
                      :to="`/admin/gejala/edit/${gejala._id}`"
                    ...
                    <!-- Delete Button -->
                    <button
                      @click="hapusGejala(gejala._id, gejala.nama)"
        ...
        <div v-if="totalPages > 1" class="bg-gradient-to-r from-gray-50/80 to-slate-50/80 backdrop-blur-sm border-t border-gray-100/50 px-6 py-4">
          <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
              Menampilkan {{ startIndex + 1 }} - {{ Math.min(endIndex, gejalas.length) }} dari {{ gejalas.length }} gejala
            </div>
            <div class="flex items-center space-x-2">
              <!-- Previous Button -->
              <button
                ...
                :class="[
                  ...
                  currentPage === 1 
                    ...
                    : 'text-gray-700 bg-white hover:bg-gradient-to-r hover:from-green-50 hover:to-emerald-50 hover:text-green-700 shadow-sm hover:shadow-md'
                ]"
              >
...
export default {
  ...
  computed: {
    totalPages() {
      return Math.ceil(this.gejalas.length / this.itemsPerPage);
    },
    startIndex() {
    ...
    paginatedGejalas() {
      return this.gejalas.slice(this.startIndex, this.endIndex);
    },
    visiblePages() {
      const delta = 2;
      const range = [];
  ...
  methods: {
    async fetchData() {
      ...
      this.error = null;
      try {
        const res = await gejalaAPI.getAll();
        this.gejalas = res.data;
        // Reset to first page when data is refreshed
        this.currentPage = 1;
      } catch (err) {
        console.error("Gagal memuat gejala:", err);
    ...
    goToPage(page) {
      if (page >= 1 && page <= this.totalPages) {
        ...
        // Collapse all expanded rows when changing page
        this.expandedRows = [];
    ...
    toggleExpand(gejalaId) {
      const index = this.expandedRows.indexOf(gejalaId);
      if (index > -1) {
        this.expandedRows.splice(index, 1);
      } else {
        this.expandedRows.push(gejalaId);
      }
    },

    async hapusGejala(id, nama) {
      // Enhanced confirmation dialog
      const result = confirm(
        `⚠️ Apakah Anda yakin ingin menghapus gejala "${nama}"?\n\nTindakan ini tidak dapat dibatalkan dan dapat mempengaruhi data diagnosa yang terkait.`
      );
      ...
      try {
        ...
        this.showSuccessMessage("Gejala berhasil dihapus dari sistem");

        // Refresh data
        await this.fetchData();

        // Adjust current page if necessary
        if (this.paginatedGejalas.length === 0 && this.currentPage > 1) {
          this.currentPage = this.currentPage - 1;
        }
      } catch (err) {
        console.error("Gagal menghapus gejala:", err);
        this.showErrorMessage("Gagal menghapus gejala. Silakan coba lagi.");
      }
    },
